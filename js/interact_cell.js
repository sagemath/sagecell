define(["jquery", "base/js/events", "interact_data", "utils"], function (
    ...
    function InteractCell(session, data, parent_block) {
        ...
        for (var name in controls) {
            if (controls.hasOwnProperty(name)) {
                this.controls[name] = new InteractData[
                    controls[name].control_type
                ](controls[name]);
            }
        }
    ...
    InteractCell.prototype.bindChange = function (cname) {
        ...
        var handler = function (event, ui) {
            ...
            if (that.controls[event.data.name].dirty_update) {
                $(that.cells[event.data.name]).addClass(
            ...
            var callbacks = {
                ...
                shell: {
                    reply: $.proxy(
                        that.session.handle_execute_reply,
                        that.session
                    ),
                },
            };
    ...
    InteractCell.prototype.placeControl = function (name) {
        ...
        div.appendChild(
            ce("div", { class: "sagecell_interactControl" }, [
                control.rendered(id),
            ])
        );
    };
    ...
    InteractCell.prototype.state = function (vals, callback) {
        if (vals === undefined) {
            ...
            for (var n in this.controls) {
                if (
                    this.controls.hasOwnProperty(n) &&
                    this.controls[n].update
                ) {
            ...
            var callbacks = {
                ...
                shell: {
                    "sagenb.interact.update_interact_reply":
                        callback ||
                        $.proxy(
                            this.session.handle_message_reply,
                            this.session
                        ),
                },
            };
    ...
    InteractCell.prototype.disable = function () {
        ...
        for (var name in this.controls) {
            if (
                this.controls.hasOwnProperty(name) &&
